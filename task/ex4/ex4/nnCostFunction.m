...
for i = 1:m
    ...
    a_3 = sigmoid(z_3);
    h_theta = a_3;    
    % make vector Y based on label (number) in y
    number = y(i);    
    Yk = eye(num_labels)(:,number);
    J += sum(-Yk.*log(h_theta) - (1-Yk).*log(1-h_theta));
...
% add regularization term and divide the result with the number of training examples (m)
J = 1/m*J + reg;    
